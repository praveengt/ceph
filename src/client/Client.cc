...
int Client::may_delete(Inode *dir, const char *name, const UserPerm& perms)
{
  ...
  if (perms.uid() != 0 && name && (dir->mode & S_ISVTX)) {
    InodeRef otherin;
    r = _lookup(dir, name, &otherin, perms);
    if (r < 0)
      goto out;
...
int Client::mkdirs(const char *relpath, mode_t mode, const UserPerm& perms)
{
  ...
  for (i=0; i<path.depth(); ++i) {
    ...
    r = _lookup(cur.get(), path[i].c_str(), &next, perms);
    if (r < 0)
      break;
    cur.swap(next);
  }
		      ...
		      Inode **out, const UserPerm& perms)
{
  ...
  r = _lookup(parent, dname, &in, perms);
  if (r < 0) {
    attr->st_ino = 0;
    goto out;
  }
...
int Client::_unlink(Inode *dir, const char *name, const UserPerm& perm)
{
  ...
  res = _lookup(dir, name, &otherin, perm);
  if (res < 0)
    goto fail;
...
int Client::_rename(Inode *fromdir, const char *fromname, Inode *todir, const char *toname, const UserPerm& perm)
{
  ...
  if (op == CEPH_MDS_OP_RENAME) {
    ...
    res = _lookup(fromdir, fromname, &oldin, perm);
    if (res < 0)
      goto fail;
    req->set_old_inode(oldin.get());
    req->old_inode_drop = CEPH_CAP_LINK_SHARED;

    res = _lookup(todir, toname, &otherin, perm);
    if (res != 0 && res != -ENOENT) {
      goto fail;
    } else if (res == 0) {
      req->set_other_inode(otherin.get());
		      ...
		      const UserPerm& perms)
{
  ...
  bool created = false;
  InodeRef in;
  int r = _lookup(parent, name, &in, perms);
